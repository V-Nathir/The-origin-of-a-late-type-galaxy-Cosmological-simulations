...
for i in snaps:
	Load=np.load((Dir[1]+'/'+i+'.npy'));Load2=np.load((Dir[2]+'/'+i+'.npy'));Load3=pd.read_csv((Dir[0]+'/'+i+'.csv')) ;Load3.head()
...
ListParticles_New=np.array(New_all_listpartCG['ID']).astype(int)
